...
export function NewAgentDialog({ onClose, onCreated }: { onClose: () => void; onCreated: (agent: Agent) => void }) {
  ...
  return <dialog ref={dialog} className="agent-dialog" aria-labelledby="new-agent-title" onCancel={onClose} onClose={onClose}>
    <form onSubmit={submit}>
      ...
      <div className="agent-dialog-preview"><WaddleAvatar size={88} color={visual.color} marking={visual.marking} cosmetics={visual.cosmetics} imageUrl={visual.imageUrl} /><span>{name.trim() || 'Seu novo agente'}<small>{roleLabel(role)}</small></span></div>
      ...
      <label>Função<select value={role} onChange={e => setRole(e.target.value)}>{AGENT_ROLE_OPTIONS.map(value => <option key={value} value={value}>{roleLabel(value)}</option>)}</select></label>
